...
<template>
  <el-form v-loading="loading" ref="form" :model="data.model" :label-width="data.labelWidth || '100px'" v-bind="data.props" :class="{'full-width': fullWidth}" :inline="isSearch" v-if="show">
    <template v-for="(item, index) in items">
      <cc-form-item :class="{'cc-form-item-full':item.full}" @blur="$emit('blur', item.prop)" @focus="$emit('focus', item.prop)" @keyup.native="$emit('keyup', item.prop)" @change="(value) => $emit('change', item.prop, value)" @halfChecked="(value) => $emit('halfChecked', item.prop, value)" @selected-obj="(value) => $emit('selected-obj', item.prop, value)" v-if="!item.if || item.if(data.model)" :model="data.model" :no-verify="isSearch" :item="item" @value-change="onValueChange" :key="item.prop || index"></cc-form-item>
    </template>
...
<script>
import CcFormItem from './DyFormItem.vue'
...
export default {
  components: { CcFormItem, CcButton },
  props: {
